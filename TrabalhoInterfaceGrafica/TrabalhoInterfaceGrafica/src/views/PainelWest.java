...
public class PainelWest extends JPanel{
	...
	public void verificarCelulasClicadasVazias(PainelTabuleiro p) {
		int somaBruta = 0;
		for (Celula celula : p.listaCelulas) {
			if(celula.getRobo() instanceof Robo && celula.getAluno() == null && celula.getBug() == null  && celula.button.botaoClicado) {
				if (!celula.jaTeveAlunoOuBug()) {
	                somaBruta += jogador.addCelulasNaoClicadas();
	            }
			}
		}
		jogador.setCelulasNaoClicadas(somaBruta);
	}
